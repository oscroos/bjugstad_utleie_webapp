...
export default async function KunderPage() {
  ...
  return (
    <main className="p-8 space-y-6">
      ...
      <Suspense fallback={<CustomersSectionLoading />}>
        <CustomersSection />
      </Suspense>
    </main>
  );
...
function CustomersSectionLoading() {
  return (
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
...
async function loadCustomers(): Promise<{ customers: Customer[]; error: AppError | null }> {
  try {
      ...
      error: normalizeError(error, {
        ...
        message:
          error instanceof Error && error.message
            ? error.message
            : "Vi kunne ikke hente kundelisten. Prov igjen litt senere.",
      }),
    };
  }
}
...
async function fetchCustomers(): Promise<Customer[]> {
  ...
  if (!response.ok) {
    ...
    throw {
      ...
      title: "Kunne ikke hente kunder",
      message: "Bjugstad API svarte med en feil. Prov igjen.",
